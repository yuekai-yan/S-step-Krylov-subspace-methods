...
AOB_options_BGS = ["CGS2", "CGS", "MGS"];
% define WB_options for BGS
WB_options_BGS = ["CGS", "CGS2", "MGS", "Householder"];
orth_option_BGS = {[3, 3]};
...
for j = 1:l
    ...
    for orth = 1:length(orth_option)
        ...
        figure(fig_orth);
        % RBGS: hollow circles + vertical error bars
        h = errorbar(0:s:s*(maxLen_orth-1), orthLoss_mean, orthLoss_ci, 'LineStyle','-', 'Marker', mk, ...
            'Color','k','MarkerSize',5,'LineWidth',1, ...
            'MarkerFaceColor','w', ...
            'DisplayName',sprintf('RBGS-GMRES(s = %d), %s/%s', s, AOB, WB));
        %h.LineStyle = 'none';    % no connecting line
    ...
    for orth = 1:length(orth_option_BGS)
        ...
        figure(fig_rel);
        % BGS
        semilogy(0:s:s*iters_BGS, relRes_BGS, 'Color',color, 'Marker', 'o', 'LineWidth', 1, ...
        ...
        figure(fig_orth);
        % BGS
        semilogy(0:s:s*iters_BGS, orthLoss_BGS, '-o', 'Color',color, 'LineWidth', 1, ...
            'DisplayName',sprintf('BGS-GMRES(s = %d), %s/%s', s, AOB_BGS, WB_BGS));
    ...
    %figure(fig_rel);
    % GMRES build-in
    %semilogy(0:s:s*(numel(resvec(1:s:end))-1),resvec(1:s:end) / norm(b), 'k-o', 'LineWidth', 1, ...
            %'DisplayName','GMRES'); 
...
%exportgraphics(gcf, sprintf('fig/%s_inv_relRes_BGS.eps', matnum));


figure(fig_orth);
xlabel('Iterations', 'FontSize', 14);
ylabel('Orthogonality Loss', 'FontSize', 14);
legend('show','Location','best');
%title(sprintf('Orthogonality loss vs. Iteration Number (m ≈ , %s)', matnum));
%exportgraphics(gcf, sprintf('fig/%s_inv_orthLoss_BGS.eps', matnum));
