# Notes – November 4, 2025
 
**Topic:** Discussion on Experimental Results of different orthogonalization methods

---


## 1. Overview of Experiment Setup

| Parameter | Description |
|------------|-------------|
| Solver | RBGS-GMRES, BGS-GMRES, GMRES(built-in)|
| Runs | 1 |
| Matrices Tested | SiO2, SiH4, Si10H16, genMatrix(1e-3), genMatrix(1e-5)
|Krylov basis| Newton basis 
|Orthogonalization of RBGS against other blocks| rCGS, rCGS2, RGS, rMGS
|Orthogonalization of RBGS within a block| rCGS, rCGS2, RGS, rMGS, rWhitening
| Metric | Relative residual, $ \left\lVert A*x - b \right\rVert / \left\lVert b \right\rVert$ |

---

## 2. Experimental Results by Matrix

---

**Minimum residual achieved by RBGS and BGS for SiO2 (AOB = rCGS2/CGS2, WB = rCGS2/CGS2, m ≈ 1001, d = 2 * m)**

- s = 5:5:60
![SiO2](SiO2_minrelative_error_vs_s.png)
- s = 60
![s60](relSiO2_s60.png)
![s60](orthSiO2_s60.png)
- s = 5:5:100
![SiO2](SiO2_minrel_s5-s100.png)
**Discussion**
- When s = 60, BGS fails due to instability in the within-block orthogonalization. In contrast, randomized methods achieve better performance because they perform the Gram–Schmidt step more robustly with the same initial basis. When s > 60, the superior performance of RBGS mainly comes from the within-block orthogonalization?



**Minimum residual achieved by RBGS and BGS for Si10H16 (AOB = rCGS2/CGS2, WB = rCGS2/CGS2, m ≈ 1001, d = 2 * m)**
- s = 5:5:50
![SiO2](Si10H16_minrelative_error_vs_s.png)
- s = 50
![s50](relSi10H16_s50.png)
![s50](orthSi10H16_s50.png)
- s = 5:5:100
![SiO2](Si10H16_minrel_s5-s100.png)



#### ➤ Orthogonalization methods for AOB and WB

###  SiO2 
**Matrix info:** \( n = 155331 \) 
**step size** \( s = 5 \)  
**Sketch info:** \( CountSketch, m ≈ 1001, d = 2 * m \)  
**Converge info:** \( ctol = 1e-16 \)  
**Krylov basis:** \( newton basis \)


s = 5, m = 801

![SiO2](relcomp_SiO2.png)
![SiO2](orthcomp_SiO2.png)




### 2.2 SiH4
**Matrix info:** \( n = 5041 \)  
**condition number:** \( 1.065536e+03 \)
**Sketch info:** \( Countsketch, m = 501, d = 2 * m \)  
**Converge info:** \( ctol = 1e-16 \)  
**Krylov basis:** \( newton basis \)

#### ➤ Orthogonalization methods for AOB and WB

![SiH4](relcomp_SiH4.png)
![SiH4](orthcomp_SiH4.png)



### 2.3 Si10H16
**Matrix info:** \( n = 17077 \)  
**condition number:** \( 5.633416e+04 \)
**Sketch info:** \( Countsketch, m = 1001, d = 2 * m \)  
**Converge info:** \( ctol = 1e-16 \)  
**Krylov basis:** \( newton basis \)

![Si10H16](relcomp_Si10H16.png)
![Si10H16](orthcomp_Si10H16.png)



### 2.3 genMatrix(1e-3)
**Matrix info:** \( n = 2500 \)  
**condition number:** \( 1e+03 \)
**Sketch info:** \( Countsketch, m = 901, d = 2 * m \)  
**Converge info:** \( ctol = 1e-16 \)  
**Krylov basis:** \( newton basis \)
n = 2500;
alpha = 0.5;
matrix = getStartMatrix(n, alpha);


![cond1e-5](relcomp_1e-3.png)
![cond1e-5](orthcomp_1e-3.png)



### 2.3 (ill-conditioned matrix) genMatrix(1e-5)
**Matrix info:** \( n = 2500 \)  
**condition number:** \( 1e+05 \)
**Sketch info:** \( Countsketch, m = 1001, d = 2 * m \)  
**Converge info:** \( ctol = 1e-16 \)  
**Krylov basis:** \( newton basis \)
n = 2500;
alpha = 0.5;
matrix = getStartMatrix(n, alpha);

(AOB = rCGS2/CGS2, WB = rCGS2/CGS2)
![cond1e-5](rel_cond1e-5.png)
![cond1e-5](orth_cond1e-5.png)

![cond1e-5](relcomp_1e-5.png)
![cond1e-5](orthcomp_1e-5.png)





### 3 Improvement of computing Hessenberg matrix
- Current Method
![currentHessenberg](currentHessenberg.png)
- Possible improvement
https://eprints.cs.univie.ac.at/8406/1/Adaptives-step_GMRES_IPDPS_2025_preprint.pdf